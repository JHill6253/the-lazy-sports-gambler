...
class Prediction(object):
    ...
    def build_model(self):
        ...
        densel_layer = tf.keras.layers.Dense(
            units=1, input_shape=([len(self.train_dataset.keys())],))
        output = densel_layer(input_layer)
        model = tf.keras.Model(inputs=input_layer, outputs=output)

        model.compile(loss="mse", optimizer=tf.keras.optimizers.Adam(
            0.01), metrics=['mae', 'mse'])
   ...
   #     print((example_batch.keys()))
    ...
    #    example_result
        ...
        history = model.fit(normed_train_data, self.train_labels,
                            epochs=EPOCHS, callbacks=[early_stop])

        # get key metrics
        loss, mae, mse = model.evaluate(
            normed_test_data, self.test_labels, verbose=0)
...
class ActiveTeamData(object):
    ...
    def getActiveTeamData(self):
        ...
        for year in range((StartYear), (LastYear)):
            print("-------------------------------------------------")
            # format the season correctly for the API
            season = str(year) + "-" + str(((year+1)-2000))
            teamLogs = teamgamelogs.TeamGameLogs(
                team_id_nullable=TeamID, season_nullable=season)  # Get the TeamLogs for the season
            print(season)
            ...
            team_abbreviation = teamLogs.get_data_frames()[
                0]["TEAM_ABBREVIATION"][0]
            ...
            for game in range(0, len(teamLogs.get_data_frames()[0])):
                ...
                opponentID = int(teams.find_team_by_abbreviation(
                    opponentAbbreviation_for_search)["id"])

                if (game % 10 == 0):
                    print("processing game {} out of {} for the {} season".format(
                        game, len(teamLogs.get_data_frames()[0]), season))

                opponentLogs = teamgamelogs.TeamGameLogs(
                    team_id_nullable=opponentID, season_nullable=season)  # get Team logs for the opposing team
                opponent_season_points_api_data = opponentLogs.get_data_frames()[
                    0]['PTS']
                ...
                print(past_games)
                print('lol')
                team_season_FT_average_raw_data.append(past_games.loc[past_games['MATCHUP'].isin(
                    ["{} @ {}".format(team_abbreviation, opponentAbbreviation), "{} vs. {}".format(team_abbreviation, opponentAbbreviation)])]["FTM"].mean())
                team_season_FG_average_raw_data.append(past_games.loc[past_games['MATCHUP'].isin(
                    ["{} @ {}".format(team_abbreviation, opponentAbbreviation), "{} vs. {}".format(team_abbreviation, opponentAbbreviation)])]["FGM"].mean())
                team_season_FG3_average_raw_data.append(past_games.loc[past_games['MATCHUP'].isin(
                    ["{} @ {}".format(team_abbreviation, opponentAbbreviation), "{} vs. {}".format(team_abbreviation, opponentAbbreviation)])]["FG3M"].mean())

                if game == 0:
                    opponent_season_point_average_raw_data.append(
                        opponent_season_points_api_data[game])
                    team_season_point_average_raw_data.append(
                        teamLogs.get_data_frames()[0]["PTS"][game])

                elif game < (len(opponentLogs.get_data_frames()[0]['PTS'])):
                    opponent_season_point_average_raw_data.append(
                        opponent_season_points_api_data.head(game).mean())
                    team_season_point_average_raw_data.append(
                        (sum(team_season_points)/len(team_season_points)))

                else:
                    opponent_season_point_average_raw_data.append(
                        opponent_season_points_api_data.mean())
                    team_season_point_average_raw_data.append(
                        (sum(team_season_points)/len(team_season_points)))
...
def compile_data_for_prediction(opp_abbrev, team_abbrev):
    ...
    season = "2020-21"
    teamLogs = teamgamelogs.TeamGameLogs(
        team_id_nullable=TeamID, season_nullable=season)
    team_season_points_api_data = teamLogs.get_data_frames()[0]["PTS"]

    team_season_point_average = [team_season_points_api_data.mean()]

    opponentID = int(teams.find_team_by_abbreviation(
        opponentAbbreviation_for_prediction)["id"])
    opponentLogs = teamgamelogs.TeamGameLogs(
        team_id_nullable=opponentID, season_nullable=season)
    ...
    team_season_FT_average = (past_games.loc[past_games['MATCHUP'].isin(
        ["{} @ {}".format(team_abbreviation, opponentAbbreviation_for_prediction), "{} vs. {}".format(team_abbreviation, opponentAbbreviation_for_prediction)])]["FTM"].mean())
    team_season_FG_average = (past_games.loc[past_games['MATCHUP'].isin(
        ["{} @ {}".format(team_abbreviation, opponentAbbreviation_for_prediction), "{} vs. {}".format(team_abbreviation, opponentAbbreviation_for_prediction)])]["FGM"].mean())
    team_season_FG3_average = (past_games.loc[past_games['MATCHUP'].isin(
        ["{} @ {}".format(team_abbreviation, opponentAbbreviation_for_prediction), "{} vs. {}".format(team_abbreviation, opponentAbbreviation_for_prediction)])]["FG3M"].mean())

    # If there is not enough data for this season, refer to the last season
    if math.isnan(team_season_FT_average):

        teamLogs = teamgamelogs.TeamGameLogs(
            team_id_nullable=TeamID, season_nullable="2019-20")
        past_games = teamLogs.get_data_frames()[0].copy()
        team_season_FT_average = (past_games.loc[past_games['MATCHUP'].isin(
            ["{} @ {}".format(team_abbreviation, opponentAbbreviation_for_prediction), "{} vs. {}".format(team_abbreviation, opponentAbbreviation_for_prediction)])]["FTM"].mean())
        team_season_FG_average = (past_games.loc[past_games['MATCHUP'].isin(
...
def game_res(team1_abbreviation, team2_abbreviation):
    ...
    team1_prediction, team1_margin = team1_prediction_class.make_prediction(
        team1_prediction_data)

    team2_prediction_data = oppData2
    team2_prediction, team2_margin = team2_prediction_class.make_prediction(
        team2_prediction_data)
    ...
    today = date.today()
    game = {'Home': team1_abbreviation, 'HomeScore': str(round((team1_prediction.item(0)+team1_margin), 0)), 'Away': team2_abbreviation, 'AwayScore': str(
        round((team2_prediction.item(0)+team2_margin), 0)), 'Date': today.strftime("%m/%d/%y"), 'HomePointAvg': str(team1ptavg), 'AwayPoint Avg': str(team2ptavg)}
    return game
